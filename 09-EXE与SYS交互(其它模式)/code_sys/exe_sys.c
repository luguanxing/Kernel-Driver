	...
	switch (psl->MajorFunction) {
	   ...
	   case  IRP_MJ_DEVICE_CONTROL: {
				...
				switch(code) {
				   case add_code: {
						int a, b, r;
						int *inputBuffer = (int*)psl->Parameters.DeviceIoControl.Type3InputBuffer;
						int *outputBuffer = (int*)pIrp->UserBuffer;
						KdPrint(("Enter add_code\n"));
						__try {
							ProbeForRead(inputBuffer, cbin, __alignof(int));
							ProbeForRead(outputBuffer, cbin, __alignof(int));
							_asm {
								mov eax, inputBuffer
								mov ebx, [eax]
								mov a, ebx
				   ...
				   case str_code: {
						...
						__try {
							...
							_asm {
								mov eax, inputBuffer
								mov ebx, [eax]
								mov mystr, ebx					//注意将缓冲区指针内容取出转化成LPCSTR，即[缓冲区指针]=字符串首地址(32位)
							}
							KdPrint(("%s\n",mystr));
							info = 0;
						} __except (EXCEPTION_EXECUTE_HANDLER) {
